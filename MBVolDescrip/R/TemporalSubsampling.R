...
temporalSubsampleMeta <- function(otus, metadata, desired_spacing, window_width, taxaAreRows = FALSE) {
  ...
  for (i in 1:length(allsubj)) {
    ...
    while (j <= length(this.times)) {
      ...
      if (any(this.times == next.time)) {
      ...
      } else if (any(abs(this.times[j:length(this.times)] - next.time) <= window_width)) {
        ...
        subseq.times <- this.times[j:length(this.times)]
        this.diffs <- abs(subseq.times - next.time)
        check.times <- which(this.diffs <= window_width)
        keep.time <- which.min(this.diffs[check.times])
        k <- check.times[keep.time]

        # update
        newmeta[which.idx[current.row], "time2"] = subseq.times[k]
        current.row = current.row + 1
        newmeta[which.idx[current.row], "time1"] = subseq.times[k]
        current.time = subseq.times[k]
        current.time.j = which(this.times == current.time)
        j <- current.time.j+1
