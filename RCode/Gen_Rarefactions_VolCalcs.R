...
for (rareprop in c("None", "P100", "P80", "P60")) {
  
  if (rareprop != "None") {
    rarepropnum = case_when(rareprop == "P100" ~ 1, 
                            rareprop == "P80" ~ 0.8, 
                            rareprop == "P60" ~ 0.6)
  }
  ...
  set.seed(1) 
  if (rareprop != "None") {
    mp_otu_rarefy <- rrarefy(t(mp_otu), sample=rarepropnum*min(mp_meta$readct))
  } else {
    mp_otu_rarefy <- rrarefy(t(mp_otu), sample=max(mp_meta$readct))
  }
  ...
  set.seed(1) 
  if (rareprop != "None") {
    smp_otu_rarefy <- rrarefy(t(smp_otu), sample=rarepropnum*min(smp_meta$readct))
  } else {
    smp_otu_rarefy <- rrarefy(t(smp_otu), sample=max(smp_meta$readct))
  }
  ...
  set.seed(1) 
  if (rareprop != "None") {
    gaj_otu_rarefy <- rrarefy(gaj_otu, sample=rarepropnum*min(gaj_meta$readct))
  } else {
    gaj_otu_rarefy <- rrarefy(gaj_otu, sample=max(gaj_meta$readct))
  }
  ...
  set.seed(1) 
  if (rareprop != "None") {
    rav_otu_rarefy <- rrarefy(rav_otu, sample=rarepropnum*500) ## min(rav_meta$readct))
  } else {
    rav_otu_rarefy <- rrarefy(rav_otu, sample=max(rav_meta$readct))
  }
